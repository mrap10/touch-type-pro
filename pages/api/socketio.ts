...
export default function handler(req: NextApiRequest, res: NextApiResponseWithSocket) {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("create_race", (payload: string | { roomId: string; username?: string }) => {
            const { roomId, username } = typeof payload === 'string' ? { roomId: payload, username: undefined } : payload;
            if (!roomId) return;
            socket.join(roomId);
            ...
            socket.emit("room_joined", {
                ...
                usersMeta: Array.from(room.usernames.entries()).map(([playerId, uname]) => ({ playerId, username: uname }))
            });
